...
class MnistBags(data_utils.Dataset):
    def __init__(self, xor_numbers=[9, 1], mean_bag_length=10, var_bag_length=2, num_bag=250, seed=1, train=True, how_much=0.5):
        ...
        assert self.how_much > 0.
        assert self.how_much < 1.
    ...
    def _create_bags(self):
        ...
        while len(bags_list) < self.num_bag:
            ...
            labels_in_bag = (torch.sum(self.xor_numbers[0] == labels_in_bag) / len(labels_in_bag)) >= self.how_much

            if labels_in_bag and positive_bags < self.num_bag // 2:
                for i in range(int(self.how_much * self.mean_bag_length) + 1):
                    indices[i] = np.asarray(np.argwhere(labels_list))[i]
                bags_list.append(all_imgs[indices])
                labels_list.append(labels_in_bag)
                positive_bags += 1

            if (not labels_in_bag) and (negative_bags < self.num_bag // 2):
                bags_list.append(all_imgs[indices])
                labels_list.append(labels_in_bag)
                negative_bags += 1
